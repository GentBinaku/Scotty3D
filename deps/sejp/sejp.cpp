...
enum Masks : uint32_t { TypeBits = 0xe0000000, IndexBits = 0x1fffffff };
...
value parse(std::istream &from) {
  ...
  auto read_char = [&from]() -> char {
    char c;
    if (!from.get(c))
      throw std::runtime_error("parse error: unexpected EOF.");
    return c;
  };
  ...
  auto read_exactly = [&read_char](std::string const &expect) {
    for (auto e : expect) {
      char c = read_char();
      if (c != e)
        throw std::runtime_error(std::string("parse error: expected '") + e +
                                 "', got '" + c + "'.");
    }
  };
  ...
  auto read_number = [&from, &read_char](char first) -> double {
    ...
    if (first == '0') {
      ...
      throw std::runtime_error(std::string("parse error: unexpected '") +
                               first + "' in number.");
    }
    ...
    if (from.peek() == '.') {
      ...
      if (!('0' <= c && c <= '9'))
        throw std::runtime_error(
            std::string("parse error: wanted fraction digits, got '") + c +
            "'.");
      acc += c;
      digits();
    }
    ...
    if (from.peek() == 'E' || from.peek() == 'e') {
      ...
      if (!('0' <= c && c <= '9'))
        throw std::runtime_error(
            std::string("parse error: wanted exponent digits, got '") + c +
            "'.");
      acc += c;
      digits();
    }
        ...
        if (c == '\\' || c == '/' || c == '"')
          ...
          for (uint32_t i = 0; i < 4; ++i) {
            ...
            if ('0' <= c && c <= '9')
              ...
              throw std::runtime_error(
                  std::string("parse error: invalid character '") + c +
                  "' in \\uNNNN escape.");
          }
          assert(value <= 0xffff);
          ...
          throw std::runtime_error(
              std::string("parse error: invalid escape '\\") + c + "'.");
        }
      } else {
  ...
  while (root.index == -1U || !parents.empty()) {
    ...
    if (parents.empty()) {
      ...
      std::map<std::string, value> &map =
          parsed->objects.at(parents.back() & IndexBits).value();
      if (!map.empty()) {
        // consume comma between entries:
        if (c != ',')
          throw std::runtime_error(
              "parse error: expected ',' between object members.");
        skip_wsp();
        c = read_char();
      }
      if (c != '"')
        throw std::runtime_error(
            "parse error: expecting '\"' at start of key.");
      std::string key = read_string();
      skip_wsp();
      c = read_char();
      if (c != ':')
        throw std::runtime_error("parse error: expecting ':' after value.");
      skip_wsp();
      ...
      std::vector<value> &array =
          parsed->arrays.at(parents.back() & IndexBits).value();
      if (!array.empty()) {
        if (c != ',')
          throw std::runtime_error(
              std::string(
                  "parse error: expected ',' between array entries; got '") +
              c + "'.");
        skip_wsp();
        c = read_char(); // actual first character of value
      }
    ...
    if (c == '{') { // object
      if (uint32_t(parsed->objects.size()) & ~IndexBits)
        std::runtime_error("parser error: too many objects.");
      ...
      if (uint32_t(parsed->arrays.size()) & ~IndexBits)
        std::runtime_error("parser error: too many arrays.");
      target->index = Array | uint32_t(parsed->arrays.size());
      parents.emplace_back(target->index);
      parsed->arrays.emplace_back(std::in_place);
      continue;
    } else if (c == '"') { // string
      if (uint32_t(parsed->strings.size()) & ~IndexBits)
        std::runtime_error("parser error: too many strings.");
      target->index = String | uint32_t(parsed->strings.size());
      parsed->strings.emplace_back(read_string());
    } else if (c == '-' || (c >= '0' && c <= '9')) { // number
      if (uint32_t(parsed->numbers.size()) & ~IndexBits)
        std::runtime_error("parser error: too many numbers.");
      ...
      throw std::runtime_error(
          std::string("parse error: value cannot start with '") + c + "'.");
    }
  }

  skip_wsp();

  if (from.peek() != std::iostream::traits_type::eof())
    throw std::runtime_error("parse error: trailing junk.");
