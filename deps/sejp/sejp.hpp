...
struct value {
  ...
  value(std::shared_ptr<parsed const> const &data_, uint32_t index_)
...
//   NOTE: loaded data is retained via shared_ptr until values referring to it
//   go out of scope NOTE: throws on parse error
