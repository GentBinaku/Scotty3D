...
public:
  ...
  Test(const std::string
           &name, // name of test case (Assignment.specific_test_name)
       const std::function<void()>
           &test // test function; throw Test::error on failure
  );
  ...
  // Throw this from a test that can't be run in the current build
  // configuration:
  ...
  static bool run_tests(
      const std::string &prefix // run test cases which include this string
  );
  ...
  differs(Halfedge_Mesh const &a, Halfedge_Mesh const &b,
          CheckExtra check_extra = CheckIdsBit);
  ...
  static bool distant_from(const Halfedge_Mesh &from, const Halfedge_Mesh &to,
                           float scale);
  static float closest_distance(const Halfedge_Mesh &from, const Vec3 &to);

  static double total_squared_error(const std::vector<double> &a,
                                    const std::vector<double> &b);
  static double total_squared_error(Spectrum a, Spectrum b);
  static double print_empirical_threshold(
      const std::vector<double> &ref,
      const std::function<std::vector<double>()> &histogram);
  static bool run_generators; // for test cases that need to run something
                              // against reference to generate data
};
